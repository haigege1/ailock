...
会话名只允许字母、数字、`-`、`_`（其余字符自动过滤）。`--notify` 加
`--sid <会话名>` 可达到同样效果；`--run` 会自动按进程号生成 sid。
详见 `AI联动接入指南.md` §1.6。

**卡片什么时候撤下来**（`core/statusdir.is_expired`）：

| 会话状态 | 撤卡 / 清理时机 |
| --- | --- |
| `waiting`（等你授权） | 不撤，一直留着 |
| 带 `watch_pid` | 不按时间撤，进程消失才结束 |
| `running` | 30 分钟没有任何更新即视为已死 |
| `done` / `failed` / `idle` | 收尾满 10 分钟 |

钩子（WorkBuddy / ZCode / Codex）在每轮结束时写 `running` + `turn_end_at`：
宿主会连坐杀掉钩子起的后台确认进程，所以 AiLock 自己按同一个「确认窗口」
（12 秒内没有新事件）把这类会话补记成 `done`，否则卡片会永远钉在锁屏上。
