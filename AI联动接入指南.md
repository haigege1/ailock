...
| `Stop` | 刷新时间戳，追加「本轮结束，等待确认…」，并写下 `turn_end_at` |

**Stop 不当场标 done**。`Stop` 在**每一轮**结束就触发，标 done 会让第一轮说完就触发"任务完成时"的动作。所以钩子先写 `running` + `turn_end_at`，12 秒确认窗口内没有新事件才收尾（多轮对话时下一轮的事件会把这个标记冲掉）。另一个兜底是**失联判定**：超过 15 分钟没有新事件 = 人走了 / 任务停了。

> 为什么收尾不靠钩子的后台确认进程：宿主（WorkBuddy / ZCode）用 Job Object 收进程树，钩子一退出那个子进程就被连坐杀掉，文件会永远停在 `running`。现在这段确认逻辑也在 AiLock 读取侧（`core/statusdir.settle`）跑，确保卡片一定会撤下来。
...
`turn_end_at` 只在「本轮结束但仍保持 running」时写：读取侧会等 12 秒确认窗口
（期间没有新事件）把它当成 `done`。自己写状态文件的脚本不必用它 —— 直接写
`done` 就行。
...
- `running` 但 30 分钟没任何更新的会话按「已死」处理，同样撤卡 + 清理
  （`waiting` 和带 `watch_pid` 的不受此限）
